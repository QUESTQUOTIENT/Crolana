/**
 * src/marketplace/marketplaceConnectors.ts
 * Marketplace connectors — Minted Network, Ebisu's Bay, OpenSea (Cronos), tofuNFT.
 * Prepares collection metadata for each marketplace, generates listing URLs.
 */
...
// ─────────────────────────────────────────────────────────────
// Generate all marketplace listings for a deployed contract
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// Collection metadata for marketplace submission
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// Minted.network API helper (auto-submit)
// ─────────────────────────────────────────────────────────────

export async function submitToMinted(submission: CollectionSubmission): Promise<{ success: boolean; message: string; url?: string }> {
  // Minted.network auto-detects deployed ERC721/ERC1155 contracts on Cronos.
  // No manual API call needed — just verify the contract URL is accessible.
...
export async function checkOpenSeaListing(contractAddress: string, chainId = 25): Promise<{ found: boolean; url: string }> {
  ...
  // OpenSea auto-indexes Cronos contracts — no manual submission needed.
  return { found: true, url };
}

// ─────────────────────────────────────────────────────────────
// NFT Transfer helper for listing (approve + list)
// ─────────────────────────────────────────────────────────────

export function getMarketplaceSeaportAddress(chainId: number): string | null {
  // OpenSea Seaport on Cronos
...
export function getMintedExchangeAddress(chainId: number): string | null {
  // Minted exchange contract on Cronos mainnet
  if (chainId === 25) return null; // Minted uses native CRO listing, no pre-approval needed
  return null;
}
