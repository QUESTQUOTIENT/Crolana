...
export function AssetCreation() {
  ...
  // ── Legendary dropzone ──────────────────────────────────────────────────
  const handleLegendaryDrop = useCallback((acceptedFiles: File[]) => {
    const newLegendaries: GeneratedNFT[] = acceptedFiles.map((file) => ({
      id: `legendary-${uuidv4()}`,
    ...
    addNotification({ type: 'success', title: 'Legendaries Added', message: `${acceptedFiles.length} legendary NFTs added.`, duration: 3000 });
  ...
  // ── Download ZIP (images + metadata skeleton) ───────────────────────────
