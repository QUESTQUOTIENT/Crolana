...
export function LiquidityManager() {
  ...
  const scanPositions = useCallback(async () => {
    ...
    try {
      ...
      // Fetch pool info for each pair in parallel (batched)
      const results = await Promise.allSettled(
        pairs.map(async ([tA, tB]) => {
          const params = new URLSearchParams({
            tokenA: tA.address, tokenB: tB.address, chainId: String(network.chainId),
          });
          const res  = await fetch(`/api/pool/info?${params}`);
          const data = await res.json();
          if (!res.ok || !data.exists) return null;
          const prov = getDexProvider(network.chainId);
          const pair = new ethers.Contract(data.pairAddress, PAIR_ABI, prov);
          const lpBal: bigint = await pair.balanceOf(walletAddress);
  ...
  const handleRemoveLiquidity = async (customLP?: string, customPercent?: number) => {
    ...
    if (!walletAddress || !window.ethereum || !pairInfo || !lpAmt) return;
    setIsTxPending(true); setError(null); setLastTx(null);
    const notifId = addNotification({ type: 'loading', title: 'Preparing…', message: 'Waiting for wallet…', duration: 0 });
    try {
      const provider = new ethers.BrowserProvider(window.ethereum);
      ...
      await ensureApproval(pairInfo.pairAddress, walletAddress, routerConfig.router, lpBn, signer, network.chainId);
      ...
      if (tokenA.isNative || tokenB.isNative) {
        ...
        tx = await router.removeLiquidity(tokenA.address, tokenB.address, lpBn, 0n, 0n, walletAddress, dl);
      }
