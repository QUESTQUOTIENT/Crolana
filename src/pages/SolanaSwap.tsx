/**
 * src/pages/SolanaSwap.tsx
 *
 * Jupiter-powered token swap for Solana.
 * Mirrors SwapPage.tsx architecture but uses Jupiter REST API instead of
 * VVS Finance router. Jupiter routes through 20+ AMMs on Solana for best
 * execution price.
 */
...
export function SolanaSwap() {
  ...
  const [tokenIn,  setTokenIn]  = useState<SolToken>(tokens[0]);   // SOL
  const [tokenOut, setTokenOut] = useState<SolToken>(tokens[1]);   // USDC
    ...
    <div className="bg-slate-800/60 rounded-xl border border-slate-700 p-4">
      ...
      <div className="flex items-center gap-3">
        <button onClick={onSelectToken}
          className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 transition-colors flex-shrink-0">
          <div className={cn('w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold text-white', token.isNative ? 'bg-gradient-to-br from-purple-500 to-pink-400' : 'bg-slate-600')}>
