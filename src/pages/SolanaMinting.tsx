/**
 * src/pages/SolanaMinting.tsx
 *
 * Metaplex-powered NFT minting launchpad for Solana.
 * Mirrors MintingDashboard.tsx (EVM) but uses:
 *  - Metaplex Token Metadata for single NFT minting
 *  - Metaplex Candy Machine v3 for collection launchpads
 *
 * The generative art + IPFS upload workflow is IDENTICAL to Cronos.
 * Only the final on-chain minting step differs.
 */
...
export function SolanaMinting() {
  ...
  const handleSingleMint = async () => {
    ...
    try {
      ...
      // Track the transaction (Gap #1)
      if (res.signature) {
        trackTx(res.signature, solanaWalletAddress, { type: 'solana_mint', mintAddress: res.mintAddress });
      }
    } catch (err: any) {
  ...
  const handleDeployCandyMachine = async () => {
    ...
    try {
      ...
      if (res.signature) {
        trackTx(res.signature, solanaWalletAddress, { type: 'candy_machine_deploy', candyMachineAddress: res.candyMachineAddress });
      }
    } catch (err: any) {
      const parsed = parseChainError(err, 'solana');
      setError(parsed.suggestion ?? parsed.message);
  ...
  const handleMintFromCM = async () => {
    ...
    try {
      ...
      updateNotification(id, { type: 'success', title: 'NFT Minted!', message: res.mintAddress.slice(0, 12) + '…' });
      if (res.signature) {
        trackTx(res.signature, solanaWalletAddress, { type: 'candy_machine_mint', mintAddress: res.mintAddress, candyMachine: cmAddress });
      }
    } catch (err: any) {
      const parsed = parseChainError(err, 'solana');
      setError(parsed.suggestion ?? parsed.message);
