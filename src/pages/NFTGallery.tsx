 ...
 * Multi-chain NFT Gallery — Cronos (EVM) + Solana
 *
 * Wires in:
 *   - getChainAdapter (Fix #1)       — unified fetch for both chains
 *   - fromEVMNFT / fromSolanaNFT (#2) — normalised UnifiedNFT display
 *   - parseChainError (#8)            — friendly error messages
 *   - /api/metadata/resolve (#6)      — normalised metadata for any URI
 *   - /api/ownership/invalidate (#11) — cache bust after mint
 */
...
export function NFTGallery() {
  ...
  // Contract address input (Cronos only)
  ...
  const loadNFTs = useCallback(async (forceRefresh = false) => {
    ...
    try {
      // Use the unified API route which goes through OwnershipSyncer (Fixes #4, #5, #11, #12)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Cronos: optional contract filter */}
      {chainTab === 'cronos' && (
