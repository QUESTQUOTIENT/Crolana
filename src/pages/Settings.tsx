...
import {
  ...
  CheckCircle, AlertCircle, Loader2, Eye, EyeOff, ArrowLeftRight,
  Sliders, Server, Palette, Sun, Moon,
} from 'lucide-react';
...
export function Settings() {
  ...
  // Switch both app state AND wallet to the selected network
  const handleNetworkSelect = async (n: Network) => {
    setNetwork(n);
    if (!window.ethereum) return; // no wallet — just update app state
    try {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-slate-400 text-sm">Configure IPFS storage, networks, DEX preferences, and server connection.</p>
      </div>
      ...
      {/* DEX Preferences */}
      <SectionCard title="DEX Preferences" icon={ArrowLeftRight}>
        <div className="space-y-5">
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5">Default Slippage</label>
            <div className="flex gap-2">
              {[10, 50, 100, 200].map((bps) => (
                <button key={bps} onClick={() => setDexPrefs((p) => ({ ...p, defaultSlippageBps: bps }))}
                  className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-colors ${dexPrefs.defaultSlippageBps === bps ? 'bg-blue-600 text-white' : 'bg-slate-950 border border-slate-800 text-slate-400 hover:text-white'}`}>
                  {bps / 100}%
                </button>
              ))}
            </div>
            <p className="text-xs text-slate-600 mt-1.5">Used as the default in Swap and Liquidity pages. 0.5% is recommended for most pairs.</p>
          </div>

          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5">Transaction Deadline</label>
            <div className="flex items-center gap-3">
              <input type="number" min={5} max={60} value={dexPrefs.defaultDeadlineMinutes}
                onChange={(e) => setDexPrefs((p) => ({ ...p, defaultDeadlineMinutes: parseInt(e.target.value) || 20 }))}
                className="w-20 bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500 outline-none" />
              <span className="text-sm text-slate-400">minutes</span>
              <p className="text-xs text-slate-600">Transactions that haven't confirmed within this window will revert.</p>
            </div>
          </div>

          <div>
            <label className="block text-xs font-medium text-slate-400 mb-3">Preferred Router</label>
            <div className="grid grid-cols-2 gap-3">
              {[
                { id: 'vvs', name: 'VVS Finance', desc: 'Primary DEX · 0x1458…', recommended: true },
                { id: 'mm',  name: 'MM Finance',  desc: 'Alt DEX · 0x1456…', recommended: false },
              ].map(({ id, name, desc, recommended }) => (
                <button key={id} onClick={() => setDexPrefs((p) => ({ ...p, preferredRouter: id as 'vvs' | 'mm' }))}
                  className={`p-4 rounded-xl border text-left transition-all ${dexPrefs.preferredRouter === id ? 'bg-blue-600/10 border-blue-500/40' : 'bg-slate-950 border-slate-800 hover:border-slate-600'}`}>
                  <div className="flex items-start justify-between mb-1">
                    <p className="text-white font-semibold text-sm">{name}</p>
                    {recommended && <span className="text-[9px] bg-green-500/20 text-green-400 border border-green-500/30 px-1.5 py-0.5 rounded font-bold">Recommended</span>}
                  </div>
                  <p className="text-slate-500 text-xs">{desc}</p>
                  {dexPrefs.preferredRouter === id && <CheckCircle className="w-4 h-4 text-blue-400 mt-2" />}
                </button>
              ))}
            </div>
          </div>

          <button onClick={() => addNotification({ type: 'success', title: 'DEX Preferences Saved', message: 'Your DEX settings are now active.', duration: 3000 })}
            ...
            <Save className="w-4 h-4" /> Save DEX Preferences
          </button>
        </div>
      </SectionCard>
      ...
      <SectionCard title="Appearance & Themes" icon={Palette}>
        <div className="space-y-6">
          ...
          {/* Preset grid — includes a Custom tile */}
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              ...
              <button
                onClick={() => {
                  // Enter custom mode — copy current theme as starting point
          ...
          {/* Custom color pickers — always visible, active when preset === 'custom' */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {([
                ...
                const applyColor = (val: string) => {
                  // Always update CSS var live — no preset check needed
                  document.documentElement.style.setProperty(cssVar, val);
                  // Update store (switches to custom preset automatically)
                  ...
                  <div key={key}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        onBlur={(e) => {
                          // Force-apply on blur even if partial
                          const val = e.target.value;
                          if (/^#[0-9a-fA-F]{6}$/.test(val)) {
                            document.documentElement.style.setProperty(cssVar, val);
                          }
            ...
            <div className="flex items-center justify-between mt-5 pt-4 border-t border-slate-800">
              ...
              <div className="flex gap-2">
                {theme.preset === 'custom' && (
                  <button
                    onClick={() => {
                      // Copy current theme colors as a new starting point
                      const currentThemeColors = { ...theme, preset: 'custom' };
                      // Re-apply all CSS vars
                      Object.entries({
