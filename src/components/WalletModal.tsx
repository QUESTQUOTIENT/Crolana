...
export function WalletModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  // Mobile detection — used to auto-trigger deep links instead of QR codes
  // (mobile users can't scan a QR on their own screen)
  const isMobile = typeof navigator !== 'undefined' &&
    /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent);

  // Feature flag: WalletConnect v3 (Reown AppKit)
  // Set VITE_ENABLE_WALLETCONNECT=true in .env + add VITE_WALLETCONNECT_PROJECT_ID
  const WC_ENABLED    = import.meta.env.VITE_ENABLE_WALLETCONNECT === 'true';
  ...
  const connectEvm = async (wallet: typeof EVM_WALLETS[0]) => {
    if (!wallet.detect()) {
      if (wallet.deepLink) {
        ...
        if (isMobile) {
          // On mobile: navigate directly into the wallet app via deep link
          window.location.href = deepLink;
        } else {
          // On desktop: show QR code to scan with mobile wallet
  ...
  const connectSolana = async (wallet: typeof SOL_WALLETS[0]) => {
    if (!wallet.detect()) {
      if (wallet.deepLink) {
        const deepLink = wallet.deepLink(siteUrl);
        if (isMobile) {
          // On mobile: open wallet app directly — user can't scan their own screen
          window.location.href = deepLink;
        } else {
          // On desktop: show QR to scan with mobile wallet
          setQrState({ url: deepLink, walletName: wallet.name, deepLink });
          setView('qr');
        }
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-slate-900 rounded-2xl border border-slate-700 shadow-2xl overflow-hidden">
        ...
        <div className="p-4 space-y-2 max-h-[55vh] overflow-y-auto">
          ...
          {/* No wallet installed warning — mobile-aware */}
          {installed.length === 0 && (
          ...
          {/* WalletConnect — shown only when VITE_ENABLE_WALLETCONNECT=true */}
          {WC_ENABLED && (
            <button
              onClick={async () => {
                if (!WC_PROJECT_ID) {
                  addNotification({ type: 'error', title: 'WalletConnect not configured',
                    message: 'Set VITE_WALLETCONNECT_PROJECT_ID in your .env file.', duration: 5000 });
                  return;
                }
                setWcPending(true);
                try {
