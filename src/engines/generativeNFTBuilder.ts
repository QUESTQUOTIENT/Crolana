/**
 * src/engines/generativeNFTBuilder.ts
 * Generative NFT collection builder engine.
 * Runs in browser — composites layers, assigns rarity, generates metadata.
 */
...
export interface TraitLayer {
  ...
  name: string;
  order: number;
  required: boolean;  // if false, trait can be "None"
  traits: TraitImage[];
}
...
export interface GeneratedNFT {
  ...
  dnaHash: string;
  rarity: number;      // 0–1 composite probability
  rarityTier: 'Legendary' | 'Epic' | 'Rare' | 'Uncommon' | 'Common';
  isLegendary: boolean;
  file?: File;         // for uploaded legendaries
}
...
function pickTrait(layer: TraitLayer, rng: () => number): TraitImage | null {
  ...
  // Add "None" weight if not required
...
// ─────────────────────────────────────────────────────────────
// Image compositing (canvas-based, browser only)
// ─────────────────────────────────────────────────────────────
...
export async function generateCollection(
  ...
  while (nfts.length - legendaries.length < toGenerate) {
    ...
    try {
      ...
      // Fallback: generate gradient placeholder if canvas fails
  ...
  // Assign rarity ranks (1 = rarest)
  const sorted = [...nfts].sort((a, b) => a.rarity - b.rarity);
  sorted.forEach((nft, rank) => { nft['rarityRank' as keyof typeof nft] = rank + 1 as any; });
...
// ─────────────────────────────────────────────────────────────
// Placeholder image generator (fallback when canvas unavailable)
// ─────────────────────────────────────────────────────────────
...
function generatePlaceholderImage(tokenId: number, collectionName: string): string {
  ...
  // Gradient background based on tokenId
...
// ─────────────────────────────────────────────────────────────
// Shuffler (post-generation randomize order)
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// Preview single NFT (for live preview in UI)
// ─────────────────────────────────────────────────────────────
