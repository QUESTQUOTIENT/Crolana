/**
 * src/engines/tokenEngine.ts
 * Full ERC-20 token creation, compilation, and deployment engine.
 * Generates OpenZeppelin-based Solidity, compiles server-side, deploys via wallet.
 */
...
export interface TokenConfig {
  ...
  isMintable: boolean;       // owner can mint more
  isBurnable: boolean;
  isPausable: boolean;
  hasPermit: boolean;        // EIP-2612 permit
  isGovernance: boolean;     // ERC20Votes
  hasTax: boolean;
  buyTaxBps: number;
  sellTaxBps: number;
  taxRecipient: string;
  maxSupply?: string;        // hard cap if mintable
  maxWalletPercent: number;  // 100 = no limit
  maxTxPercent: number;      // 100 = no limit
...
export async function deployToken(
  ...
  // Constructor args depend on features
