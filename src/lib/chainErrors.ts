/**
 * src/lib/chainErrors.ts
 * ─────────────────────────────────────────────────────────────
 * CHAIN-SPECIFIC ERROR HANDLING  (Fix #8)
 *
 * Parses raw errors from EVM (Cronos) and Solana into user-friendly
 ...
 *     showNotification({ type: 'error', title: parsed.title, message: parsed.message });
 *   }
 */
...
export interface ParsedChainError {
  ...
  isRetryable: boolean;   // should "retry" be shown?
  isFatal: boolean;       // is this unrecoverable?
...
function parseEVMError(message: string, data: unknown, raw: unknown): ParsedChainError {
  ...
  // Contract not found / bad address
...
function extractRevertReason(message: string, data: unknown): string | null {
  // Try to extract human-readable revert string from message
  const match = message.match(/reverted[^:]*:\s*(.+?)(?:\s*\(|$)/i);
  if (match) return match[1].trim();

  // ABI-decoded revert reason from data (0x08c379a0 = Error(string))
...
/**
 * useChainErrorHandler — convenience hook for components
 *
 * @example
 *   const { handleError } = useChainErrorHandler('cronos');
 *   try { await mint(); }
