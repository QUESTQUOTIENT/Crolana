...
export async function getBestRouterQuote(
  ...
  const results = await Promise.allSettled(
    routers.map(async (r) => {
      const amounts = await getSwapQuote(r.address, amountIn, path, provider);
      return { amounts, router: r.address, routerName: r.name, amountOut: amounts[amounts.length - 1] };
    }),
  );

  const successful = results
    .filter((r): r is PromiseFulfilledResult<MultiRouterQuote> => r.status === 'fulfilled')
    .map(r => r.value);

  if (successful.length === 0) throw new Error('All routers failed to return a quote');

  // Return the router with the best (highest) output amount
  return successful.reduce((best, cur) => cur.amountOut > best.amountOut ? cur : best);
}
