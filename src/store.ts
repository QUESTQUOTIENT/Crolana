  ...
  try {
    ...
    if (saved) {
      ...
      if (parsed.bgBase && parsed.accentPrimary) {
        // Back-fill bgRaised for themes saved before it was added
...
export const useAppStore = create<AppState>((set) => ({
  ...
  // ALL notifications auto-dismiss after 5 s.
  // Exception: loading toasts (duration: 0) stay until updateNotification
  // resolves them — then they also auto-dismiss after 5 s.
  ...
  updateNotification: (id, updates) => {
    ...
    // When a loading toast resolves to success / error / info, auto-dismiss it
