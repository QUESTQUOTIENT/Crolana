...
export const MAINNET_TOKENS: Token[] = [
  ...
  },
  {
    ...
    name: 'USD Coin',
    decimals: 6,
    logoUrl: 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/cronos/assets/0xc21223249CA28397B4B6541dfFaEcC539BfF0c59/logo.png',
  },
  {
  ...
  },
  {
    ...
    name: 'Dai Stablecoin',
    decimals: 18,
    logoUrl: 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/cronos/assets/0xF2001B145b43032AAF5Ee2884e456CCd805F677D/logo.png',
  },
  {
...
export function getTokenList(chainId: number): Token[] {
  const defaultTokens = chainId === 25 ? MAINNET_TOKENS : TESTNET_TOKENS;
  ...
  for (const t of defaultTokens) addToken(t);
  for (const t of customTokens) addToken(t);
...
export function getCustomTokens(chainId: number): Token[] {
  try {
    const key = `crolana-custom-tokens-${chainId}`;
...
export function saveCustomToken(chainId: number, token: Token) {
  try {
    const key = `crolana-custom-tokens-${chainId}`;
...
export function removeCustomToken(chainId: number, tokenAddress: string) {
  try {
    const key = `crolana-custom-tokens-${chainId}`;
    const existing = getCustomTokens(chainId);
    const filtered = existing.filter(t => t.address.toLowerCase() !== tokenAddress.toLowerCase());
