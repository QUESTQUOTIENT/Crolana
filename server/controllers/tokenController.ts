...
/**
 * Extract the first `contract Foo` name from Solidity source.
 * Fallback when the client sends a null/empty contractName.
 */
...
export const compileTokenContract = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (source && typeof source === 'string' && source.trim().length > 0) {
      ...
      // contractName may arrive as null/undefined/empty — extract from source as fallback
      finalContractName = (contractName && typeof contractName === 'string' && contractName.trim())
        ? contractName.trim()
        : extractContractName(finalSource);
    } else if (config && typeof config === 'object') {
      // Path 2: generate from config on the server side
    ...
    // Sanitise contractName — must be a valid Solidity identifier
