/**
 * server/services/txTracker.ts
 * ─────────────────────────────────────────────────────────────
 * TRANSACTION STATUS TRACKING  (Fix #7)
 *
 * Reliable pending → confirmed → failed tracking for both:
 *   - Cronos (EVM): polls block confirmations
 *   - Solana: polls signature status with backoff
 *
 * Usage:
 *   const tracker = TxTracker.getInstance();
 *   const record  = await tracker.track({ chain: 'solana', txHash: signature, walletAddress: pk });
 *   const status  = await tracker.getStatus(record.id);
 */
...
export interface TxRecord {
  ...
  fee?: string;
  errorMessage?: string;
  metadata?: Record<string, unknown>; // e.g. { type: 'mint', tokenId: 1 }
  createdAt: number;       // ms
  updatedAt: number;       // ms
...
export class TxTracker {
  ...
  /** Start tracking a transaction. Returns the TxRecord immediately. */
  ...
  /** Get all tracked transactions for a wallet. */
  ...
  /** Get a transaction by its hash (most recent if duplicates). */
  ...
  private startPolling(
    ...
    let attempts = 0;
    const maxAttempts = record.chain === 'solana' ? 60 : 40; // Solana faster
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (result.status !== 'pending') {
          ...
          this.clearPoller(id);
          // Persist final status to DB (Gap #1)
  ...
  /** Purge confirmed/failed records older than `maxAgeMs` (default 1 hour) */
  ...
  // ── DB Persistence (Gap #1) ──────────────────────────────────────────────

  /**
   * Persist a TxRecord to the TrackedTransaction DB model.
   * Non-blocking — called fire-and-forget from the /tx/track route.
   * Silently no-ops if Prisma is unavailable.
   */
  async persistToDb(record: TxRecord): Promise<void> {
    try {
      ...
      // Silently fail — in-memory tracker is the source of truth at runtime
    }
  }

  /**
   * Update DB record whenever status changes to confirmed/failed.
   * Called internally after polling resolves.
   */
