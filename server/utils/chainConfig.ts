/**
 * server/utils/chainConfig.ts
 * ─────────────────────────────────────────────────────────────
 * ENV SPLIT PER CHAIN  (Gap #6 — clean runtime config helper)
 *
 * Single source of truth for all chain-specific runtime config.
 * Reads the split env vars from .env and provides typed access.
 *
 * Usage:
 *   import { cronosConfig, solanaConfig, getChainConfig } from '../utils/chainConfig';
 *   const rpc = cronosConfig.rpc;        // always correct for active network
 *   const cfg = getChainConfig('solana'); // dynamic lookup
 */
...
/** Active RPC for a given chain — resolves to the correct mainnet/testnet endpoint */
export function getActiveRpc(chain: SupportedChain): string {
  return chain === 'cronos' ? cronosConfig.rpc : solanaConfig.rpc;
}

/** Active block explorer URL for a given chain */
...
/** Log config on startup (masks API keys) */
