/**
 * /api/storage/* — v6 NFT Engine storage API
 *
 * Pipeline:
 *   POST /storage/uploadAsset     → upload single image, returns CID
 *   POST /storage/uploadMetadata  → queue metadata folder upload, returns jobId
 *   GET  /storage/getCID/:jobId   → poll job, returns CID when done
 */
...
const upload = multer({
  ...
  limits: { fileSize: 100 * 1024 * 1024 }, // 100 MB for large collections
});
